...
# References:# [1] Peacock, J. A. (1983). Two-dimensional goodness-of-fit testing in
# astronomy. Monthly Notices of the Royal Astronomical Society, 202(3),
# 615-627.
...
# (1992). Numerical recipes in C. Press Syndicate of the University of
...
def CountQuads(Arr2D, point):
    """ Computes the probabilities of finding points in each 4 quadrant defined
        by a vertical and horizontal lines crossing the point, by counting
    ...
    **Returns:**
        ``fpp``, ``fnp``, ``fpn``, ``fnn`` : float
            The probabilities of finding a point in each quadrants, with point
            as the origin.  p stands for positive, n for negative, with the
            first and second positions meaning the x and y directions
            respectively.
    """
    ...
    if Arr2D.shape[1] > Arr2D.shape[0]:
        # Reshape so that A[row,column] is achieved.
        Arr2D = Arr2D.copy().T
    if Arr2D.shape[1] != 2:
    ...
    logging.debug('Number of points in each quadrant: Qpp=' +
                  str(len(Qpp))+'Qnp='+str(len(Qnp))+'Qpn=' +
                  str(len(Qpn))+'Qnn='+str(len(Qnn)))
    ...
    fpn = len(Qpn)*ff
    fnn = len(Qnn)*ff
    logging.debug('Probabilities of finding points in each quadrant: fpp=' +
                  str(fpp)+'fnp='+str(fnp)+'fpn='+str(fpn)+'fnn='+str(fnn))
    ...
    # cause SOMETIMES sum to 1.000000002 or something. I don't know how
    # to test for that reliably, OR what to do about it yet. Keep in mind.
...
def FuncQuads(func2D, point, xlim, ylim, rounddig=4):
    """ Computes the probabilities of finding points in each 4 quadrant defined
        by a vertical and horizontal lines crossing the point, by integrating
        the density function ``func2D`` in each quadrant.
    ...
    **Arguments:**
        ...
        ``xlim``, ``ylim`` : list or ndarray
            Domain of numerical integration necessary to compute the quadrant
            probabilities.
    **Returns:**
        ``fpp``, ``fnp``, ``fpn``, ``fnn`` : float
            The probabilities of finding a point in each quadrants, with point
            ...
            respectively.
    """
    logging.info('FuncQuads function. Computes the probability of finding\
                 points in quadrants around a point using a 2D density\
                 function.')
    ...
    totInt = scipy.integrate.dblquad(func2D, *xlim, lambda x: np.amin(ylim),
    ...
    logging.info('Probabilities of finding points in each quadrant: fpp=' +
                 str(fpp)+' fnp='+str(fnp)+' fpn='+str(fpn)+' fnn='+str(fnn))
    logging.debug('Total probability, which should be equal to one: ' +
                  str(fpp+fnp+fpn+fnn))
...
def Qks(alam, iter=100, prec=1e-6):
    """ Computes the value of the KS probability function, as a function of
        ...
        '[...] the K–S statistic useful is that its distribution in the
        case of the null hypothesis (data sets drawn from the same
        distribution) can be calculated, at least to useful approximation,
        thus giving the significance of any observed nonzero value of D.'
        (D being the KS statistic).
    ...
    **Arguments:**
        ...
        ``iter``:
            ...
            On non-convergence, returns 1.0.
        ``prec``:
            Convergence criteria of the qks. Stops converging if
            that precision is attained.
    **Returns:**
        ``qks`` : float
            The significance level of the observed D statistic.
    """
    # If j iterations are performed, meaning that to add is still 2 times
    # larger than the precision.
    logging.info('Qks function. Value of the KS probability function\
                 using a float.')
    ...
    while (j < iter) & (abs(toadd[-1]) > prec*2):
        ...
        j += 1
    if (j == iter) | (qks > 1):
        #  If no convergence after the j iterations, return 1.0.
...
def ks2d2s(Arr2D1, Arr2D2):
    """ ks stands for Kolmogorov-Smirnov, 2d for 2 dimensional,
        2s for 2 samples.
        KS test for goodness-of-fit on two 2D samples. Tests the hypothesis
    ...
    **Returns:**
        ``d`` : float
            The two-sample K-S statistic. If this value is higher than
            the significance level of the hypothesis, it is rejected.
        ``prob`` : float
            The significance level of ``d``. Small values of prob show
...
def ks2d1s(Arr2D, func2D, xlim=[], ylim=[]):
    ...
    **Arguments:**
        ...
        ``xlim``, ``ylim`` : list or ndarray
            Defines the domain for the numerical integration necessary
            to compute the quadrant probabilities.
    **Returns:**
        ``d`` : float
            The two-sample K-S statistic. If this value is higher than
            the significance level of the hypothesis, it is rejected.
...
def MultiVarNHPPThinSamples(lambdaa, Intervals, Samples=100, blocksize=1000):
    # Utility function: generate spatial data by thinning.
    # Intervals is a np array of 2 lenght lists. lambdaa is a function or
    # 2d matrix.Iterate over blocksize uniformlly generated points until
    # Samples number of samples are created.
    logging.info('NHPP samples in space by thinning. lambda can be a\
                 2D matrix or function')
    ...
    if callable(lambdaa):
        ...
        max = scipy.optimize.minimize(lambda x: -lambdaa(*x),
                                      x0=[np.mean(i) for i in Intervals],
                                      bounds=boundstuple)
        lmax = lambdaa(*max.x)
    else:
